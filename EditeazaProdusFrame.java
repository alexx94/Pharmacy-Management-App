...
public class EditeazaProdusFrame extends JFrame{
    private JTextField indexText, nt, ct, pt, compozitieText, indicatieText, contraindicatieText;
    private JButton adauga;
    private AscultatorC a;

    public EditeazaProdusFrame() {
        super ("Editarea unui produs din stoc");
        var p=new JPanel(new GridLayout(7,2,10,10));
        p.setBackground(new Color(204, 236, 238));	
        
        p.add(new JLabel("Cod/Index"));
        ...
        adauga = new JButton("Modifica Produs");
        ...
        public void actionPerformed(ActionEvent e){
            if (e.getSource() == adauga) {
                if (!nt.getText().equals("") && !ct.getText().equals("") && !pt.getText().equals("") && !compozitieText.getText().equals("") && !indicatieText.getText().equals("") && !contraindicatieText.getText().equals("")) {
                    cp.editeazaProdus(Integer.parseInt(indexText.getText()), nt.getText(), Integer.parseInt(ct.getText()), Double.parseDouble(pt.getText()), compozitieText.getText(), indicatieText.getText(), contraindicatieText.getText());
                }
                
            }
            EditeazaProdusFrame.this.dispose();
        }
    }
}
